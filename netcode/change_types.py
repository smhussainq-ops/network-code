...
def _rollback_routing_redistribution(intent: RoutingRedistributionIntent) -> str:
    ...
    return (
        f"router {item.to_protocol} {item.target_process}\n"
...
register(ChangeTypeSpec(
    ...
    title=lambda i: f"Configure {i.redistribution.from_protocol.upper()} to {i.redistribution.to_protocol.upper()} redistribution",
    ...
    blast_objects=lambda i: [
        f"{i.redistribution.from_protocol.upper()}→{i.redistribution.to_protocol.upper()} boundary",
        f"Route-map {i.redistribution.route_map}",
        f"Prefix-list {i.redistribution.prefix_list}",
    ],
    checks=lambda i: {
        "pre": [
            {
        ...
        "post": [
            {
                ...
                "description": f"OSPF process {i.redistribution.target_process} references route-map {i.redistribution.route_map}.",
                "executable": True,
            }
        ],
    },
    verification_hint=lambda i: {
        ...
        "params": {
            "target_protocol": i.redistribution.to_protocol,
            "target_process": i.redistribution.target_process,
            "route_map": i.redistribution.route_map,
        },
    },
    ...
    block_carveouts=["router ospf", "redistribute bgp", "route-map", "ip prefix-list"],
))
